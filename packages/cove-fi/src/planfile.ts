 ...
 * integers and pass through TOML natively. RETIREMENT (-2, `income.end`
 * only) gets nicer TOML sugar: `dumpPlan` writes it as the string
 * `"retirement"`, and `loadPlan` maps `income.end = "retirement"` back to
 * -2 before validation — everywhere else in the plan, `"retirement"` is
 * just an invalid value for a numeric field.
 *
...
import { DEFAULT_ASSUMPTIONS, type Plan, RETIREMENT } from "./model.js";
...
/** `income.end = "retirement"` (TOML sugar) -> the RETIREMENT (-2) sentinel `planFromJson` expects; every other `incomes[]` field passes through untouched. */
...
/** `[plan].birth_year` in TOML -> top-level `birth_year` in the JSON shape `planFromJson` expects; every other key passes through untouched (except `incomes[].end`, see `mapRetirementStrings`). */
function reshapeFromToml(parsed: unknown): unknown {
  ...
  return { birth_year, ...rest, incomes: mapRetirementStrings(rest.incomes) };
}
...
export function dumpPlan(plan: Plan): string {
  const tomlShape = {
    ...
    incomes: plan.incomes.map((i) => (i.end === RETIREMENT ? { ...i, end: "retirement" } : i)),
...
const TEMPLATE_PLAN: Plan = {
  ...
  incomes: [{ name: "salary", amount: 75000, start: 2026, end: RETIREMENT }],
  social_security: [],
...
# runs from/until the coast-fi trigger") in the design doc. An income's
# \`end = "retirement"\` means "no fixed date either — end automatically the
# year before assumptions.retirement_year," so a retirement-year scenario
