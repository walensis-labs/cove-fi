...
2. **Opting a cash account into the new return model taxes its
   interest.** If you set `assumptions.class_returns.cash` or an
   individual cash account's `ret`, that account's resolved growth is now
   taxed as ordinary income every year (working and retired) — real
   interest-income treatment. Untouched or legacy-`growth`-only cash
   accounts are unaffected and stay untaxed, matching 0.3.0 exactly; this
   is opt-in only.
